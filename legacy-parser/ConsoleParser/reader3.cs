...
namespace ConsoleParser
{
    class reader3
    {
        ...
        public static List<Zaniatie> MainReader(DataSet ds)
        {
            ...
            for (int str = FindDate(ds)[0]; str <= FindEnd(ds); str += FindHeight(ds))//организация прыжка по клеткам вертикали на основе высоты
                for (int stl = FindDate(ds)[1]; stl < FindEndStlb(ds); stl += FindWidth(ds))//организация прыжка по клеткам по горизонтали на основе ширины
                {
                    string bufer = "";
                    string gruppa = "";
                    ...
                    if (bufer != "")//если клетка была не пустая, добавляем туда информацию о группе (из строки) и времени (из столбца) 
                    {
                        ...
                        */
                        ...
                        date = getDate(ds, str);
                        List_Training.Add(new Zaniatie(gruppa, date, Time, bufer, subgroup, true));
                    }
                }
        ...
        /// </summary>
        /// <param name="ds">dataser, где требуется найти группу</param>
        ...
        {
            return ds.Tables[0].Rows[FindGrupp(ds)[0] - 1][stlb - 1].ToString();
            /*
            string currentGroup = "";
        ...
        private static DateTime getDate(DataSet ds, int str)
        {
            ...
            foreach (DataTable table in ds.Tables)//таблица
            {
                ...
                {
                    intRow++;
                    string znach = row[FindDate(ds) - 1].ToString();
                    if ((znach == "Понедельник") || (znach == "Вторник") || (znach == "Среда") || (znach == "Четверг") || (znach == "Пятница") || (znach == "Суббота"))
                    {
                        if (intRow > str)
                            return Previous_Date;
                        Previous_Date = table.Rows[intRow][FindDate(ds) - 1].ToString();
                    }
                    if (isDate(znach))
                    {
                        if (Previous_Date == "")
            ...
            return Previous_Date;
            */
            object poss = ds.Tables[0].Rows[str-1][FindDate(ds)[1]-1];
            //Console.WriteLine(poss.ToString());
            if (double.TryParse(poss.ToString(), out double Possible_date))
            {
                try
                {
                    DateTime OUTdate = DateTime.FromOADate(Possible_date);
                    return OUTdate;
                }
                catch (Exception)
                {
        ...
        /// <summary>
        /// Найти последнюю строку (Основываясь на дате)
        /// </summary>
        /// <param name="ds">В каком dataset</param>
        ...
        */
        ...
        /// <summary>
        /// Найти координаты первой клетки с датой
        /// </summary>
        /// <param name="ds">В каком dataset</param>
        ...
        {
            if (!isTime(znach) && double.TryParse(znach, out double Possible_date))
            {
                try
                {
                    DateTime OUTdate = DateTime.FromOADate(Possible_date);
